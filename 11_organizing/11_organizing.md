...
- some general tips:
    ...
    - for naming results, include some reference to the source data from which the results were generated, as well as the name of the analysis that was run on that data to generate that result
...
- we've been mostly executing short code fragments in IPython/Jupyter
    ...
    - one very simple way of organizing things would be to have one Jupyter notebook per experiment, each notebook generating results for that experiment only
...
- `git add`
    - let's say you create a file in your project folder
...
- `git clone`
    ...
    - `git clone git@github.com:SciPyCourse2017/notes.git` clones the class notes to your current working directory
